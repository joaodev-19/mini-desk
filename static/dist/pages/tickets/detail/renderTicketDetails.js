...
function renderStatusBadge(element, status, statusDisplay) {
    ...
    element.classList.remove("status-open", "status-in_analysis", "status-waiting_user", "status-resolved", "status-closed");
    element.classList.add(`status-${status}`);
}
export function renderSummaryCard(container, data) {
    ...
    const ticketStatus = container.querySelector("#ticket-detail-status");
